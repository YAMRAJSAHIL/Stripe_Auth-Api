...
def check_card():
    ...
    try:
        ...
        if ajax_nonce is None:
            return jsonify({"error": "Could not find setup intent nonce"}), 400
        ...
        if "error" in stripe_response:
            ...
            if "your request was in live mode, but used a known test card" in error_message:
                return jsonify({"result": "𝗗𝗲𝗰𝗹𝗶𝗻𝗲𝗱 ❌ | your card was declined. your request was in live mode, but used a known test card."})          
            else:
                return jsonify({"result": f"𝗗𝗲𝗰𝗹𝗶𝗻𝗲𝗱 ❌ | {stripe_response['error']['message']}"})
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5000, debug=True)            'sec-fetch-dest': 'document',
            'sec-fetch-mode': 'navigate',
            'sec-fetch-site': 'none',
            'sec-fetch-user': '?1',
            'upgrade-insecure-requests': '1',
